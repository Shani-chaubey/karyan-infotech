...
export default function HomePortalForm() {
  ...
  if (!data) {
    return <p className="text-sm text-stone-500">Loading your home page…</p>;
  }
  ...
  return (
    <div className="space-y-8">
      <CmsPageIntro
        ...
        where="Everything here appears on the public home page at “/” — the first page visitors see."
      >
        Each block below matches a section on the site. Open a block, change the words or images, then press
        “Save home page” once at the bottom. You never need to touch code or special symbols.
      </CmsPageIntro>
          ...
          {data.heroSlides.map((slide, i) => (
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <CmsField label="Internal project key" hint="For your team only — links analytics to a project.">
                  <CmsInput
                    value={slide.project}
                    onChange={(e) =>
                      patch((d) => {
              ...
              <CmsField
                ...
                hint="Paste the page path, e.g. /karyan-trevana — where the button should go."
              >
                <CmsInput
              ...
              <CmsField
                ...
                hint="Paste an image address (URL). Your designer or hosting panel can supply this."
              >
                <CmsInput
                  value={slide.bg}
                  onChange={(e) =>
                    patch((d) => {
                      const heroSlides = [...d.heroSlides];
                      heroSlides[i] = { ...heroSlides[i], bg: e.target.value };
                      return { ...d, heroSlides };
                    })
                  }
                />
      ...
      <CmsSection
        ...
        description="The four bold numbers in a row (signature developments, corridors, etc.)."
      ...
      >
        <div className="space-y-4">
          {data.statCards.map((card, i) => (
            ...
            >
              <CmsField label="Large number or text" hint="e.g. 4+">
                <CmsInput
                  value={card.value}
                  onChange={(e) =>
                    patch((d) => {
              ...
              <CmsField label="Subtitle" hint="Smaller line under the title.">
                <CmsInput
                  value={card.sub}
                  onChange={(e) =>
                    patch((d) => {
      ...
      <CmsSection
        title="Philosophy"
        description="Your positioning paragraph — tone of voice for the brand."
        where="Home page — editorial block with badge and long text."
        defaultOpen={false}
      >
        <CmsField label="Small badge" hint="Word above the main heading, e.g. “Philosophy”.">
          <CmsInput
            value={data.philosophy.badge}
            onChange={(e) => patch((d) => ({ ...d, philosophy: { ...d.philosophy, badge: e.target.value } }))}
          />
        </CmsField>
        <CmsField label="Section heading">
          <CmsInput
            value={data.philosophy.title}
            onChange={(e) => patch((d) => ({ ...d, philosophy: { ...d.philosophy, title: e.target.value } }))}
          />
        </CmsField>
        <CmsField label="Body copy">
          <CmsTextarea
            value={data.philosophy.body}
            onChange={(e) => patch((d) => ({ ...d, philosophy: { ...d.philosophy, body: e.target.value } }))}
          />
        </CmsField>
      </CmsSection>
      ...
      <CmsSection
        title="Capabilities"
        description="Intro line plus the four service pillars with icons."
      ...
      >
        ...
        <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Capability cards</p>
        <div className="space-y-4">
          {data.capabilities.map((c, i) => (
            <CmsItemCard
              ...
              title={`Card ${i + 1}`}
              onRemove={() =>
                patch((d) => ({ ...d, capabilities: d.capabilities.filter((_, j) => j !== i) }))
              }
            >
              <CmsField label="Icon name" hint="Internal icon key (e.g. Layers, Gem) — ask your web team if unsure.">
                <CmsInput
                  value={c.icon}
                  onChange={(e) =>
                    patch((d) => {
      ...
      <CmsSection
        ...
        description="Heading and the four project tiles that deep-link to detail pages."
        where="Home page — “Portfolio” grid linking to Trevana, CityWalk, etc."
        defaultOpen={false}
      >
        ...
        <div className="space-y-4">
          {data.portfolio.map((p, i) => (
            <CmsItemCard
              ...
              onRemove={() => patch((d) => ({ ...d, portfolio: d.portfolio.filter((_, j) => j !== i) }))}
            >
      ...
      <CmsSection
        title="Why Karyan"
        description="Section intro and three pillars."
      ...
      >
        ...
        <CmsField label="Heading">
          <CmsInput
            ...
            onChange={(e) => patch((d) => ({ ...d, whyIntro: { ...d.whyIntro, title: e.target.value } }))}
          />
        </CmsField>
        <div className="space-y-4">
          {data.pillars.map((p, i) => (
            <CmsItemCard
              key={i}
              title={`Pillar ${i + 1}`}
              onRemove={() => patch((d) => ({ ...d, pillars: d.pillars.filter((_, j) => j !== i) }))}
            >
      ...
      <CmsSection
        title="Commitments"
        description="Heading and bullet list of promises."
        where="Home page — checklist-style commitments."
        defaultOpen={false}
      >
        <CmsField label="Eyebrow">
          <CmsInput
      ...
      <CmsSection
        title="Process steps"
        description="The four numbered steps (Discover → Handover)."
        where="Home page — horizontal “Simple steps” timeline."
        defaultOpen={false}
      >
        <CmsField label="Eyebrow">
          <CmsInput
        ...
        <div className="space-y-4">
          {data.process.map((step, i) => (
            <CmsItemCard
              key={i}
              title={`Step ${i + 1}`}
              onRemove={() => patch((d) => ({ ...d, process: d.process.filter((_, j) => j !== i) }))}
            >
      ...
      >
        ...
        <CmsField label="Badge text" hint="Small label on the section.">
          <CmsInput
        ...
        <div className="space-y-4">
          {data.testimonials.map((t, i) => (
            <CmsItemCard
              ...
              title={`Quote ${i + 1}`}
              onRemove={() =>
                patch((d) => ({ ...d, testimonials: d.testimonials.filter((_, j) => j !== i) }))
              }
            >
              <CmsField label="Quote">
                <CmsTextarea value={t.quote} onChange={(e) => patch((d) => {
      ...
      >
        ...
        <CmsField label="Heading">
          <CmsInput
            ...
            onChange={(e) => patch((d) => ({ ...d, faqIntro: { ...d.faqIntro, title: e.target.value } }))}
          />
        </CmsField>
        <div className="space-y-4">
          {data.faqs.map((faq, i) => (
            <CmsItemCard
              key={i}
              title={`Question ${i + 1}`}
              onRemove={() => patch((d) => ({ ...d, faqs: d.faqs.filter((_, j) => j !== i) }))}
            >
          ...
          <CmsGhostButton
            onClick={() => patch((d) => ({ ...d, faqs: [...d.faqs, { q: "", a: "" }] }))}
          >
            + Add question
          </CmsGhostButton>
        </div>
      ...
      <CmsSection
        ...
        description="Corridor narrative, bullets, and small corridor table."
      ...
      >
        ...
        <CmsField label="Corridors table title">
          <CmsInput
            ...
            onChange={(e) =>
              patch((d) => ({ ...d, location: { ...d.location, corridorsTitle: e.target.value } }))
            }
          />
        </CmsField>
        <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Highlights with icons</p>
        <div className="space-y-4">
        ...
        <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Corridor rows</p>
        <div className="space-y-4">
          {data.location.corridors.map((c, i) => (
            ...
            >
              ...
              <CmsField label="Projects text" hint="Shown beside the corridor name.">
                <CmsInput
                  value={c.projects}
                  onChange={(e) =>
                    patch((d) => {
      ...
      <CmsSection
        ...
        description="Heading for the blog strip and up to three teaser lines."
        ...
        <div className="space-y-4">
          ...
          <CmsGhostButton
            onClick={() =>
              patch((d) => ({
                ...d,
                journalTeasers: [...d.journalTeasers, { title: "", href: "/blog", hint: "" }],
              }))
            }
          >
            + Add teaser
      ...
      >
        <CmsField label="Eyebrow">
          <CmsInput
            ...
            onChange={(e) =>
              patch((d) => ({ ...d, ecosystem: { ...d.ecosystem, eyebrow: e.target.value } }))
            }
          />
        </CmsField>
        <CmsField label="Body">
      ...
      <CmsSection
        ...
        where="Home page — wide two-column strip above the final banner."
        defaultOpen={false}
      >
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-3 rounded-xl border border-stone-100 bg-stone-50/50 p-4">
            <p className="text-xs font-bold uppercase text-stone-500">Left — phone</p>
            <CmsField label="Eyebrow">
              <CmsInput
            ...
            <CmsField label="Phone number (display)">
              <CmsInput
                ...
                onChange={(e) =>
                  patch((d) => ({ ...d, splitCta: { ...d.splitCta, phone: e.target.value } }))
                }
              />
            </CmsField>
            <CmsField label="Phone link" hint="Usually tel:+919206001002">
              <CmsInput
            ...
            <CmsField label="Hours line">
              <CmsTextarea
                ...
                onChange={(e) =>
                  patch((d) => ({ ...d, splitCta: { ...d.splitCta, hours: e.target.value } }))
                }
          ...
          <div className="space-y-3 rounded-xl border border-stone-100 bg-stone-50/50 p-4">
            <p className="text-xs font-bold uppercase text-stone-500">Right — enquiry</p>
            <CmsField label="Title">
              <CmsInput
      ...
      <div className="sticky bottom-4 z-10 rounded-2xl border border-stone-200 bg-white/95 p-4 shadow-lg backdrop-blur">
        <CmsOnSiteBadge text="Saving updates the live home page for all visitors (after refresh)." />
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <CmsPrimaryButton onClick={save}>Save home page</CmsPrimaryButton>
